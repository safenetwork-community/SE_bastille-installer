...
// Dialog box texts
const DBOX_TEXT_USERNAME: &str = "Enter the username you want: \n\
    (usernames must be all lowercase and first character may not be a number)"; 
const DBOX_TEXT_USERGROUPS_1: &str = "Enter additional groups besides the default groups which are\n\
    --> wheel,sys,audio,input,video,storage,lp,network,users,power <--\n\
    for user \'";
const DBOX_TEXT_USERGROUPS_2: &str = "\' in a comma seperated list:"; 


// Message box texts
const DBOX_TEXT_USERNAME_INVALID: &str = "Username contains invalid characters\n\
    \n\
    \t\tPlease try again";
const DBOX_TEXT_USERNAME_EMPTY: &str = "Username cannot be empty\n\
    \n\
    \t\tPlease try again";
...
impl App {
    ...
    pub fn run(&mut self) -> Result<(), Error> {
        ...
        loop {
            match current_box {
                ...
                Some(USERGROUP) => current_box = Self::handle_dbox_usergroups(self),
    ...
    fn handle_dbox_username(&mut self) -> Option<&'static str> {
        match Self::dbox(90, 10, &DBOX_TEXT_USERNAME) {
            Some(name) => {
                if name.is_empty() {
    ...
    fn handle_dbox_usergroups(&mut self) -> Option<&'static str> {
        match Self::dbox(90, 11, 
            &[DBOX_TEXT_USERGROUPS_1, &self.username, DBOX_TEXT_USERGROUPS_2].concat()) {
            Some(_) => {
                return None
            },
            None => return Some(CANCEL),
    ...
    fn handle_mbox_username_empty() -> Option<&'static str> {
        ...
        dbox.set_width(40);
        dbox.set_height(7);
        dialog::Message::new(DBOX_TEXT_USERNAME_EMPTY)
            .show_with(&dbox)
            .expect(EXP_MBOX);
        Some(USERNAME)
    }
    ...
    fn handle_mbox_username_invalid() -> Option<&'static str> {
        ...
        dbox.set_width(40);
        dbox.set_height(7);
        dialog::Message::new(DBOX_TEXT_USERNAME_INVALID)
            .show_with(&dbox)
            .expect(EXP_MBOX);
        Some(USERNAME)
    }
    ...
    // Dialog box
    fn dbox(width: u32, height: u32, text: &str) -> Option<String> {
        let mut dbox = dialog::backends::Dialog::new();
        dbox.set_backtitle(TITRFOQ);
